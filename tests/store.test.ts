...
const recipe = recipeSchema.parse({ title: 'Lemon pasta', ingredients: [{ quantity: 200, unit: 'g', ingredient: 'spaghetti' }, { quantity: 1, ingredient: 'lemon' }], steps: [{ text: 'Boil pasta. Toss with lemon.' }], tags: ['Weeknight'] });
test('private collections, fuzzy ingredients, title-first ranking, tags and indexed updates', () => {
  const store = new Store(':memory:');
  try {
    const alice = store.ensureUser('alice@example.com'); const bob = store.ensureUser('bob@example.com');
    const created = store.mutate(alice.id, { action: 'create', data: recipe }, 'human');
    assert.equal(created.status, 'applied'); if (created.status !== 'applied') return;
    ...
    store.mutate(alice.id, { action: 'create', data: { ...recipe, title: 'Spaghetti supper' } }, 'human');
    assert.equal(store.search(alice.id, 'spaghetti')[0].title, 'Spaghetti supper');
    assert.equal(store.search(bob.id).length, 0);
    assert.throws(() => store.get(bob.id, created.recipe.id), (e: unknown) => e instanceof AppError && e.status === 404);
    assert.throws(() => store.mutate(bob.id, { action: 'delete', recipeId: created.recipe.id, baseVersion: 1 }, 'human'));
    store.mutate(alice.id, { action: 'update', recipeId: created.recipe.id, baseVersion: 1, data: { ...recipe, title: 'Tomato rice', ingredients: [{ quantity: 1, unit: 'cup', ingredient: 'rice', note: '' }] } }, 'human');
...
test('agent proposals require review, enforce ownership and detect stale edits', () => {
  ...
  try {
    ...
    assert.equal(pending.status, 'pending'); if (pending.status !== 'pending') return;
    ...
    const edit = store.mutate(alice.id, { action: 'update', recipeId: approved.id, baseVersion: 1, data: { ...recipe, title: 'Agent version' } }, 'agent', true);
    store.mutate(alice.id, { action: 'update', recipeId: approved.id, baseVersion: 1, data: { ...recipe, title: 'Human version' } }, 'human');
    if (edit.status !== 'pending') throw new Error('Expected pending');
    assert.throws(() => store.review(alice.id, edit.changeId, true), (e: unknown) => e instanceof AppError && e.status === 409);
...
test('per-user YOLO, atomic merge, soft deletion, immutable history and restore', () => {
  ...
  try {
    ...
    assert.equal(store.mutate(bob.id, { action: 'create', data: recipe }, 'agent', true).status, 'pending');
    ...
    const merge = { action: 'merge', recipeId: first.recipe.id, baseVersion: 1, sourceId: second.recipe.id, sourceVersion: 2, data: { ...recipe, title: 'Merged lemon pasta' } };
    ...
    assert.equal(restored.version, 3); assert.equal(restored.title, recipe.title);
